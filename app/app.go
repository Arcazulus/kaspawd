...
var serviceDescription = &winservice.ServiceDescription{
	Name:        "kaspawdsvc",
	DisplayName: "Kaspawd Service",
	Description: "Downloads and stays synchronized with the Kaspaw blockDAG and " +
		"provides DAG services to applications.",
}

type kaspawdApp struct {
	cfg *config.Config
}

// StartApp starts the kaspawd app, and blocks until it finishes running
func StartApp() error {
...
func (app *kaspawdApp) main(startedChan chan<- struct{}) error {
	...
	if err != nil {
		log.Errorf("Unable to start kaspawd: %+v", err)
		return err
	}

	defer func() {
		log.Infof("Gracefully shutting down kaspawd...")
		...
		log.Infof("Kaspawd shutdown complete")
	}()
