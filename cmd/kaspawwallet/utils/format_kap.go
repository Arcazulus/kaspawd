...
// FormatKap takes the amount of sompis as uint64, and returns amount of KAP with 8  decimal places
func FormatKap(amount uint64) string {
	res := "                   "
	if amount > 0 {
		res = fmt.Sprintf("%19.8f", float64(amount)/constants.SompiPerKaspaw)
	}
	return res
}

// KapToSompi takes in a string representation of the Kap value to convert to Sompi
func KapToSompi(amount string) (uint64, error) {
	err := validateKAPAmountFormat(amount)
	...
	if constants.SompiPerKaspaw%10 != 0 {
		return 0, errors.Errorf("Unable to convert to sompi when SompiPerKaspaw is not a multiple of 10")
	}

	decimalPlaces := int(math.Log10(constants.SompiPerKaspaw))
	decimalStr := ""
...
func validateKAPAmountFormat(amount string) error {
