...
import (
	...
	"github.com/Arcazulus/kaspawd/domain/consensus/utils/subnetworks"
...
func sweep(conf *sweepConfig) error {
	...
	publicKeybytes, err := libkaspawwallet.PublicKeyFromPrivateKey(privateKeyBytes)
	if err != nil {
		return err
	}
	...
	UTXOs, err := libkaspawwallet.KaspawwalletdUTXOsTolibkaspawwalletUTXOs(getExternalSpendableUTXOsResponse.Entries)
	if err != nil {
		return err
	}
	...
	for i, txID := range response.TxIDs {
		fmt.Printf("\t%s\n", txID)
		fmt.Println("\tSwept:\t", utils.FormatKap(splitTransactions[i].Outputs[0].Value), " KAP")
		totalExtracted = totalExtracted + splitTransactions[i].Outputs[0].Value
	}

	fmt.Println("\nTotal Funds swept (including transaction fees):")
	fmt.Println("\t", utils.FormatKap(totalExtracted), " KAP")
