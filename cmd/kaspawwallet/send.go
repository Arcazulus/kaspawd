...
func send(conf *sendConfig) error {
	...
	if !conf.IsSendAll {
		sendAmountSompi, err = utils.KapToSompi(conf.SendAmount)
	...
	for i, unsignedTransaction := range createUnsignedTransactionsResponse.UnsignedTransactions {
		signedTransaction, err := libkaspawwallet.Sign(conf.NetParams(), mnemonics, unsignedTransaction, keysFile.ECDSA)
		if err != nil {
			return err
		}
