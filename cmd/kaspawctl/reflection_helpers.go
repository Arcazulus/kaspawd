...
// 2. A wrapper that implements iskaspawdMessage_Payload, having a single field pointing to the concrete command
//    bearing the name of the command with `kaspawdMessage_` prefix and `Request` suffix
...
// generatekaspawdMessage generates a wrapped kaspawdMessage with the given `commandValue`
func generateKaspawdMessage(commandValue reflect.Value, commandDesc *commandDescription) (*protowire.KaspadMessage, error) {
	commandWrapper := reflect.New(commandDesc.typeof)
	unwrapCommandValue(commandWrapper).Set(commandValue)

	kaspawdMessage := reflect.New(reflect.TypeOf(protowire.KaspadMessage{}))
	kaspawdMessage.Elem().FieldByName("Payload").Set(commandWrapper)
	return kaspawdMessage.Interface().(*protowire.KaspawdMessage), nil
}
