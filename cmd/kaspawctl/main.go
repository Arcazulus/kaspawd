...
import (
	...
	"github.com/Arcazulus/kaspawd/infrastructure/network/rpcclient/grpcclient"
)
...
func main() {
	...
	if !cfg.AllowConnectionToDifferentVersions {
		kaspawdMessage, err := client.Post(&protowire.KaspawdMessage{Payload: &protowire.KaspawdMessage_GetInfoRequest{GetInfoRequest: &protowire.GetInfoRequestMessage{}}})
		if err != nil {
			printErrorAndExit(fmt.Sprintf("Cannot post GetInfo message: %s", err))
		}

		localVersion := version.Version()
		remoteVersion := kaspawdMessage.GetGetInfoResponse().ServerVersion
...
func prettifyResponse(response string) string {
	kaspawdMessage := &protowire.KaspawdMessage{}
	err := protojson.Unmarshal([]byte(response), kaspawdMessage)
	if err != nil {
	...
	return marshalOptions.Format(kaspawdMessage)
}
